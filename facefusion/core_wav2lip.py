...
def cli(target_path,execution_thread_count,execution_queue_count,execution_providers="gpu",frame_processors='face_enhancer') -> None:
  ...
  #other，兼容wav2lip，此处提示多余的unrecognized arguments，引入即可
	...
	program.add_argument('--wav2lip_batch_size', dest = 'wav2lip_batch_size')
	program.add_argument('--crop', dest = 'crop')
	program.add_argument('--box', dest = 'box')
	program.add_argument('--rotate', dest = 'rotate')
	program.add_argument('--nosmooth', dest = 'nosmooth')
	program.add_argument('--no_seg', dest = 'no_seg')
	program.add_argument('--no_sr', dest = 'no_sr')
	program.add_argument('--sr_model', dest = 'sr_model')
	program.add_argument('--fullres', dest = 'fullres')
	program.add_argument('--debug_mask', dest = 'debug_mask')
	program.add_argument('--preview_settings', dest = 'preview_settings')
	...
	group_execution.add_argument('--execution-providers', help = wording.get('execution_providers_help').format(choices = 'cpu'), dest = 'execution_providers', default = [ execution_providers ], choices = encode_execution_providers(onnxruntime.get_available_providers()), nargs = '+')
	group_execution.add_argument('--execution-thread-count', help = wording.get('execution_thread_count_help'), dest = 'execution_thread_count', type = int, default = execution_thread_count)
	group_execution.add_argument('--execution-queue-count', help = wording.get('execution_queue_count_help'), dest = 'execution_queue_count', type = int, default = execution_queue_count)
	...
	group_frame_processors.add_argument('--frame-processors', help = wording.get('frame_processors_help').format(choices = ', '.join(available_frame_processors)), dest = 'frame_processors', default = [ frame_processors ], nargs = '+')
...
def apply_args(program : ArgumentParser) -> None:
	...
	if isinstance(facefusion.globals.target_path, list):
...
#专门多线程处理wav2lip生成的cv2帧
def run_wav2lip(program : ArgumentParser) -> None:
	apply_args(program)
	limit_resources()
	if not pre_check():
		return
